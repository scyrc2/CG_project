...
Sky::Sky(const int& gridX, const int& gridZ, const std::string& filename)
...
{
	float x, y, z;
	glPushAttrib(GL_ALL_ATTRIB_BITS);              // Set the material properties of the Skys surface
		...
		glPushMatrix();
			glTranslatef(0.0, 50.0, 0.0);   // position the Sky
			//glTranslatef(pos[0], pos[1], pos[2]);   // position the Sky
			glRotated(180.0, 0.0, 0.0, 1.0);
			//glScalef(scale[0], scale[1], scale[2]); // scale the unit Sky
			glScaled(100.0, 1.0, 100.0); 

			y = 0.0f; z = -0.5f; // initialise the start position as the top left coorner of the unit Sky mesh
			glBegin(GL_QUADS);                                      // draw the unit Sky surface as quads
...
void Sky::Update(const double& deltaTime)
{
	...
	for (int j = 0; j <= zGridDims; j++)
	{
		for (int i = 0; i <= xGridDims; i++)
		{
			// if the Sky is not frozen then calculate texCoord based on the sample position + some amount offset by a spherical function
