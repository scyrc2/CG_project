...
Camera::Camera() : wKey(0), sKey(0), aKey(0), dKey(0), currentButton(0), mouseX(0), mouseY(0), fKey(0), gKey(0)
{
	Reset();
}
...
void Camera::Reset(){
	...
	//camera position (28,5,0)
	eyePosition[0] = 28.0f;
	eyePosition[1] = 5.0f;
	eyePosition[2] = 0.0f;

	//view direction, originally (-1,0,0)
	vd[0] = -1.0f;
	vd[1] = 0.0f;
	vd[2] = 0.0f;
...
void Camera::Update(const double& deltaTime)
{
	...
	//viewpoint change
	//After switching the viewpoint,the camera position, view direction and movement directio  should be updated
	if (zKey)
	{
		eyePosition[0] = 0.0f;
		eyePosition[1] = 5.0f;
